...
class VideoDDIMSampler(BaseDiffusionSampler):
    ...
    def denoise(self, x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None, context_image=None):
        additional_model_inputs = {}
        
        if context_image!=None:
            # additional_model_inputs['context_image'] = torch.cat((context_image, context_image), 0)
            raise NotImplementedError("must process context_image for cfg")
        else:
            raise Exception("must specify context_images for inference")
...
class VPSDEDPMPP2MSampler(VideoDDIMSampler):
    ...
    def sampler_step(
        ...
        denoised = self.denoise(
            x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep, idx, scale=scale, scale_emb=scale_emb, context_image=context_image
        ).to(torch.float32)
        if idx == 1:
    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, scale=None, scale_emb=None, context_image=None):
        x, s_in, alpha_cumprod_sqrt, num_sigmas, cond, uc, timesteps = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
...
class VPSDEDPMPP2MSampler_cfg(VPSDEDPMPP2MSampler):
    
    def denoise(self, x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None, context_image=None):
        additional_model_inputs = {}
        
        if context_image!=None:
            # repeat context_image for cfg
            additional_model_inputs['context_image'] = torch.cat((context_image, context_image), 0)
        else:
            raise Exception("must specify context_images for inference")
        if isinstance(scale, torch.Tensor) == False and scale == 1:
