...
class SATVideoDiffusionEngine(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        not_trainable_prefixes = model_config.get("not_trainable_prefixes", ["first_stage_model", "conditioner", "context_image_encoder.siglip"])
        ...
        self.ref_image_dropout = model_config.get("ref_image_dropout", 0.0)
    ...
    def shared_step(self, batch: Dict) -> Any:
        ...
        if batch['face_image'] is not None:
            images = batch['face_image'].to(self.dtype).permute(0, 2, 1, 3, 4).contiguous()
            context_images = []
            for i in range(images.shape[2]):
                image = self.add_noise_to_first_frame(images[:,:,i:i+1,:,:])
                context_image = self.encode_first_stage(image, batch).permute(0, 2, 1, 3, 4)
                context_images.append(context_image)
            context_images = torch.cat(context_images, dim=1)
            if random.random() < self.ref_image_dropout:
                context_images = torch.zeros_like(context_images)
            batch["context_image"] = context_images
    ...
    def sample(
        ...
        **kwargs,
        ...
        samples = self.sampler(denoiser, randn, cond, uc=uc, scale=scale, scale_emb=scale_emb, context_image=context_image)
    ...
    def log_video(
        ...
        **kwargs,
        ...
        if self.noised_image_input:
            ...
            c["concat"] = image
            uc["concat"] = image
            samples = self.sample(c, shape=z.shape[1:], uc=uc, batch_size=N, context_image=context_images, **sampling_kwargs)  # b t c h w
