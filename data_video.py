...
class FaceJsonMultiPerSFTDataset(Dataset):
    def __init__(self, data_dir, video_size, fps, max_num_frames, skip_frms_num=3):
        """
        skip_frms_num: ignore the first and the last xx frames, avoiding transitions.
        """
    ...
    def __getitem__(self, index):
        ...
        if ori_vlen / actual_fps * self.fps > self.max_num_frames:
            num_frames = self.max_num_frames
            start = int(self.skip_frms_num)
            end = math.ceil(start + num_frames / self.fps * actual_fps) - int(self.skip_frms_num)
            end_safty = min(end, int(ori_vlen) - int(self.skip_frms_num)) 
            indices = np.arange(start, end, max((end - start) // num_frames, 1)).astype(int)
            temp_frms = vr.get_batch(np.arange(start, end_safty)) 
            assert temp_frms is not None
            tensor_frms = torch.from_numpy(temp_frms) if type(temp_frms) is not torch.Tensor else temp_frms 
        ...
        if self.face_images[index]!="":
            if os.path.isdir(self.face_images[index]): # for single identity 
                face_image = sorted( [os.path.join(self.face_images[index], name) for name in os.listdir(self.face_images[index]) if name.endswith(('png', 'jpg', 'jpeg'))])
                face_image = self.transform(Image.open(np.random.choice(face_image)).convert('RGB')).unsqueeze(dim=0) #[1, C, H, W]
                face_image = resize_pad_crop(face_image, self.video_size)
                face_image = (face_image * 2.0 - 1.0).clip(-1,1)
            else: 
                if '@@' not in self.face_images[index]: # for single identity 
                    ...
                    face_image = [resize_pad_crop(temp_face, self.video_size) for temp_face in face_image]                    
